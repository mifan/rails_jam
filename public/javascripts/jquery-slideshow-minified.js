...
(function($){
	...
	$.fn.slideShow = function(options) {
		...
		this.init = function() {
			...
			this.find('.first, .next, .prev, .last, .slide-navigation, .slide, .page, .slides').data('slideShow', this);
			...
			this.find('.slide-navigation .page:eq(' + this.current + ')').addClass('selected');
			this.find('.page').click(function() {
				...
				if (!(index = parseInt($(this).html()-1))) {
					var index = $(this).parents('.slide-navigation').find('.page').index($(this));
				}
		...
		this.gotoSlide = function(index, noanimation) {
			...
			oldFinished = function () {
				...
				slideShow.elm.find('.slide-navigation .page:eq(' + slideShow.current + ')').addClass('selected');
			...
			newFinished = function() {
				...
				slideShow.elm.find('.slide-navigation .page:not(:eq(' + slideShow.current + '))').removeClass('selected');
...
$(function(){
  $('.callbackSlideShow').slideShow(
    { hoverNavigation: false, // use mouse for navigation 
